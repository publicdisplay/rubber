...
namespace :rubber do
  ...
  def refresh_instance(instance_alias)
    ...
    instance_terminating = instance_found && response.reservationSet.
      item[0].instancesSet.item[0].instanceState.name == "terminating"

    if !instance_found
      logger.info "Cannot find #{instance_alias}; deleting from configuration"
      rubber_cfg.instance.remove(instance_alias)
    elsif instance_terminating
      logger.info "#{instance_alias} terminating; deleting from configuration"
      rubber_cfg.instance.remove(instance_alias)
